...
- **Payload**: Inject the `cat /etc/passwd` command.
  ...
  cat /etc/passwd
  ```
- **Review**: Examine the web application’s response for a list of user accounts. Each line typically represents a user in a colon-separated format denoting user information like username, user ID, group ID, home directory, and default shell.
