...
- **System.Diagnostics.Process.Start**: Frequently used to invoke OS commands from within C#/VB.NET code by setting `ProcessStartInfo` parameters such as `FileName` and `Arguments`.
...
- This payload injects C# code into an XSLT stylesheet, allowing OS command execution and output retrieval.

- **Execution:**
  - Submit this payload as the XSLT input to the vulnerable endpoint processing XSLT.

- **Expected Output:**

```
text block
Output will contain the result of the executed command (e.g., current user).
```
...
```
text block
Output will display command execution result.
```
...
```
text block
Output will show PowerShell command results.
```
