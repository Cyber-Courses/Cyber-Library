...
### Symbol Limitations and Bypass Techniques

In addition to blocking certain functions, WAFs may also limit the use of specific symbols, such as commas or parentheses, which are common in SQL queries. To navigate these limitations, attackers can employ alternative syntax or encoding techniques to reduce symbol usage and obfuscate their SQL payloads, thereby bypassing these restrictions.
...
### Symbol Limitation Bypass

- **Identify symbol restrictions** that the WAF imposes by analyzing errors or failed queries upon injection attempts.
- Use alternative syntax in your SQL query to reduce symbol usage, adapting your payload to the limitations. An example command is:
  
  ```sql
  SELECT JSON_ARRAYAGG(column_name) FROM table_name WHERE condition LIMIT 1;
  ```

- **Adapt your payload** using alternative methods, such as changing the structure or encoding parts of your query, to further obfuscate and bypass the WAF's symbol analyzers.

The objective is to bypass symbol limitations using alternative syntax or format strategies, ensuring your query executes despite imposed restrictions.
