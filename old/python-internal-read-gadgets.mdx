...
In this example, `dir(obj)` lists all the names in the object’s scope, including special methods and the instance variables `a` and `b`. On the other hand, `vars(obj)` returns a dictionary of the instance variables.
